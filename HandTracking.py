# =========================
# Two-Hand Gesture Mouse (Headless, Extra-Stable Cursor)
# - One hand = cursor
# - Other hand = gestures (click/drag, right-click, scroll)
# OpenCV + MediaPipe + Kalman + Adaptive Smoothing + Smooth Scroll
# =========================
...
# Kalman smoothing (cursor) — tuned for steadier hover
KALMAN_PROCESS_VAR = 18.0   # lower = smoother hover (was 30)
KALMAN_MEAS_VAR    = 9.0    # higher = trusts measurement less (was 6)

# Extra cursor stabilizers (post-Kalman)
MOVE_DEADZONE_PX   = 6      # ignore tiny jitters below this
MOVE_MAX_STEP_PX   = 45     # clamp any per-frame jump
MOVE_EMA_ALPHA_MIN = 0.10   # smoothing when moving slowly
MOVE_EMA_ALPHA_MAX = 0.35   # smoothing when moving fast
SPEED_LOW_PX_S     = 180    # speed (px/s) for MIN alpha
SPEED_HIGH_PX_S    = 900    # speed (px/s) for MAX alpha
...
def fingers_extended_by_angle(lm_px, handed="Right"):
    ...
    lateral_ok = (lm_px[4][0] < lm_px[3][0]) if handed == "Right" else (lm_px[4][0] > lm_px[3][0])
    ext0 = (ang < 42.0) and lateral_ok
    # Index..Pinky
    joints = [(5,6,8), (9,10,12), (13,14,16), (17,18,20)]
    ext = [ext0, False, False, False, False]
...
def main():
    ...
    try:
        while True:
            ...
            if res.multi_hand_landmarks:
                for idx, hand_lms in enumerate(res.multi_hand_landmarks):
                    label = res.multi_handedness[idx].classification[0].label  # "Left"/"Right"
                    lm_px = [(int(lm.x*w), int(lm.y*h)) for lm in hand_lms.landmark]
                    sc    = hand_scale(lm_px)
                    nx, ny = hand_lms.landmark[8].x, hand_lms.landmark[8].y
            ...
            # --------------- CURSOR MOVE (Kalman + adaptive smoothing) ---------------
            if use_cursor is not None:
                ...
                kx, ky = map(int, kf.get())

                # Adaptive EMA based on speed
                if last_cursor is None:
                    smx, smy = kx, ky
                else:
                    dx = kx - last_cursor[0]
                    dy = ky - last_cursor[1]
                    speed = math.hypot(dx, dy) / max(1e-3, dt)  # px/s

                    if speed <= SPEED_LOW_PX_S:
                        alpha = MOVE_EMA_ALPHA_MIN
                    elif speed >= SPEED_HIGH_PX_S:
                        alpha = MOVE_EMA_ALPHA_MAX
                    else:
                        t = (speed - SPEED_LOW_PX_S) / (SPEED_HIGH_PX_S - SPEED_LOW_PX_S)
                        alpha = MOVE_EMA_ALPHA_MIN * (1 - t) + MOVE_EMA_ALPHA_MAX * t

                    smx = int((1 - alpha) * last_cursor[0] + alpha * kx)
                    smy = int((1 - alpha) * last_cursor[1] + alpha * ky)

                # Micro-deadzone + clamp jumps
                if last_cursor is None:
                    outx, outy = smx, smy
                else:
                    if math.hypot(smx - last_cursor[0], smy - last_cursor[1]) < MOVE_DEADZONE_PX:
                        outx, outy = last_cursor
                    else:
                        dx = smx - last_cursor[0]
                        dy = smy - last_cursor[1]
                        step = math.hypot(dx, dy)
                        if step > MOVE_MAX_STEP_PX:
                            scale = MOVE_MAX_STEP_PX / step
                            outx = int(last_cursor[0] + dx * scale)
                            outy = int(last_cursor[1] + dy * scale)
                        else:
                            outx, outy = smx, smy

                last_cursor = (outx, outy)

                # Only move if index finger is up on cursor hand (prevents incidental motion)
                ext_c = fingers_extended_by_angle(use_cursor["lm_px"], handed=CURSOR_HAND)
                if ext_c[1]:  # index up
                    pyautogui.moveTo(outx, outy)
            ...
            if use_gesture is not None:
                ...
                def dist(a,b):
                    return math.hypot(lm_px_g[a][0]-lm_px_g[b][0], lm_px_g[a][1]-lm_px_g[b][1])
                ...
                # Right click (with simple cooldown)
        ...
        if left_down:
            pyautogui.mouseUp()
        grab.release()
        # headless: no cv2.destroyAllWindows()
